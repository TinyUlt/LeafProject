...
// String is a convenience method for calling Reply.Bytes() and converting it to string
...
func (r *Reply) ListUint32() ([]uint32, error) {
	// Doing all this in two places instead of just calling ListBytes() so we don't have
	// to iterate twice
	if r.Type == ErrorReply {
		return nil, r.Err
	...
	us := make([]uint32, len(r.Elems))
	for i, v := range r.Elems {
		if v.Type == BulkReply {
			v, _ := strconv.Atoi(string(v.buf))
			us[i] = uint32(v)
		} else if v.Type == NilReply {
			us[i] = 0
		} else {
...
func (r *Reply) Values(in ...interface{}) error {
	if r.Type == ErrorReply {
		return r.Err
	}
	if r.Type != MultiReply {
		return errors.New("reply type is not MultiReply")
	}
	if len(r.Elems) < len(in) {
		return errors.New("redis: Scan multibulk short")
	}
	var err error
	for i, d := range in {
		switch d := d.(type) {
		case *string:
			*d, err = r.Elems[i].String()
		case *int:
			*d, err = r.Elems[i].Int()
		case *int32:
			var t int
			...
			*d = int32(t)
		case *int64:
			*d, err = r.Elems[i].Int64()
		case *uint32:
			var t int64
			t, err = r.Elems[i].Int64()
			*d = uint32(t)
		case *float64:
			*d, err = r.Elems[i].Float64()
		case *float32:
			var t float64
			t, err = r.Elems[i].Float64()
			*d = float32(t)
		case *bool:
			*d, err = r.Elems[i].Bool()
		case *[]byte:
			*d, err = r.Elems[i].Bytes()
		default:
			panic("redis values cant conv this type")
		}
		if err != nil {
			return err
		}
...
// Use method Reply.String() for reading string reply.
