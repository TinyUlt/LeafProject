...
func (s *Server) routeHandler(req *http.Request, w http.ResponseWriter) (unused *route) {
	...
	ctx.SetHeader("Server", "rock-texas", true)
	tm := time.Now().UTC()

	//ignore errors from ParseForm because it's usually harmless.

	req.ParseForm()
	//_log.Debug(">>>>>>>> routeHandler %v", len(req.Form))
	//_log.Debug("###### routeHandler %v", *req)
	if len(req.Form) > 0 {
		for k, v := range req.Form {
			ctx.Params[k] = v[0]
			//_log.Debug(">>>>>>> routeHandler %v", v[0])
		}
	}
	...
	//Set the default content-type                 application/x-www-form-urlencoded; text/html; charset=utf-8
	ctx.SetHeader("Content-Type", "application/x-www-form-urlencoded; text/html; charset=utf-8", true)
